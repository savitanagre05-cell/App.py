...
st.markdown("""
    ...
    .main-card { background: white; padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); border: 1px solid #eee; margin-bottom: 20px; }
    .category-label { background: #00416A; color: white; padding: 5px 15px; border-radius: 5px; margin: 15px 0; font-weight: bold; }
    .stButton>button { background-color: #00416A !important; color: white !important; width: 100%; font-weight: bold; }
    ...
    .payment-instruction { background-color: #fff3cd; color: #856404; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold; border: 1px solid #ffeeba; margin-top: 10px; }
    </style>
    ...
    if menu == "Logout":
        ...
        if st.session_state.selected_car is None:
            ...
            if pay == "PhonePe Scanner":
                # स्कॅनर इमेज लोड करण्याचा सुरक्षित प्रयत्न
                try:
                    st.image("1000327329.png", width=300)
                except:
                    st.warning("स्कॅनर फोटो लोड होत नाहीये.")

                st.markdown(f"""
                    <div class="payment-instruction">
                        📞 PhonePe Number: 9309146504<br>
                        📸 पेमेंट केल्यावर स्क्रीनशॉट व्हॉट्सॲपवर पाठवा.
                    </div>
                """, unsafe_allow_html=True)
                utr_input = st.text_input("UTR नंबर टाका")
            ...
            if st.button("Confirm ✅"):
                if p_up and d_off:
                    ...
                    msg = (
                        f"*नवीन बुकिंग - Balaji Logistics*\n"
                    ...
                    # WhatsApp मेसेज जुन्याच नंबरवर (9767981986) जाईल
                    url = f"https://wa.me/919767981986?text={urllib.parse.quote(msg)}"
                    st.success("बुकिंग यशस्वी!")
                    st.markdown(f'<a href="{url}" target="_blank" class="wa-btn">WhatsApp वर पाठवा</a>', unsafe_allow_html=True)
