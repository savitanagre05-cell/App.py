...
QR_OWNER = "Mr ANIL NARHARI NAGARE"
...
""", unsafe_allow_html=True)
        ...
        if pay == "Online":
            ...
            <div style="background:#181818; padding:20px; border-radius:15px; text-align:center; border:2px solid #5f259f; max-width:400px; margin:auto;">
                ...
                <h4 style="color:white; margin:5px 0;">{QR_OWNER}</h4>
        ...
        if pw == ADMIN_PASS:
            ...
            if not df.empty:
                ...
                if st.button("Update Booking Details"):
                    df.loc[df["booking_id"] == selected_bid, "driver_name"] = d_name if d_name else "Not Assigned"
                    df.loc[df["booking_id"] == selected_bid, "driver_mobile"] = d_mob if d_mob else "Not Assigned"
                    df.loc[df["booking_id"] == selected_bid, "status"] = new_status
                    df.to_csv(BOOKING_DB, index=False)
                    st.success(f"Booking {selected_bid} updated successfully!")
                    st.rerun()