...
st.markdown("""
    ...
    .main-card { background: white; padding: 25px; border-radius: 20px; box-shadow: 0 8px 20px rgba(0,0,0,0.1); border: 1px solid #e0e0e0; margin-bottom: 25px; }
    .car-card { background: white; padding: 10px; border-radius: 15px; text-align: center; margin-bottom: 5px; }
    .stButton>button { background-color: #00416A !important; color: white !important; border-radius: 10px; width: 100%; font-weight: bold; border: none; padding: 12px; height: 50px; }
    label, p, span { color: #333 !important; font-weight: 600; }
    .wa-btn { background-color: #25D366; color: white; padding: 15px; border-radius: 12px; text-align: center; display: block; text-decoration: none; font-weight: bold; font-size: 16px; margin-top: 15px; }
    .car-price { color: #28a745; font-size: 18px; font-weight: bold; margin-bottom: 10px; }
    h3 { margin-bottom: 2px; color: #00416A; }
    </style>
...
if not st.session_state.logged_in:
    # लोगो फाईल नसल्यास एरर येऊ नये म्हणून चेक करा
    logo_path = "1000326575.png" # तुझ्याकडे ही फाईल नसल्यास, ती काढून टाक किंवा योग्य पाथ द्या
...
# --- पायरी २: मुख्य ॲप ---
    ...
    if menu == "🔓 Logout":
        ...
        if st.session_state.selected_car is None:
            ...
            pay = st.radio("पेमेंट मोड:", ["Cash", "PhonePe Scanner"], horizontal=True)
            utr = ""
            if pay == "PhonePe Scanner":
                # *** तुझा QR कोड इथे लोड होईल ***
                st.image("image_0.png", width=300, caption="Scan & Pay to Shree Balaji Logistic")
