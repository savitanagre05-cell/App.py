...
# डेटाबेस फाइल्स चेक करणे (cust_mob कॉलमसह)
if not os.path.isfile(USER_DB): pd.DataFrame(columns=['username', 'password', 'mobile']).to_csv(USER_DB, index=False)
if not os.path.isfile(BOOKING_DB): pd.DataFrame(columns=['username', 'date', 'from_loc', 'to_loc', 'vehicle', 'fare', 'pay_mode', 'cust_mob']).to_csv(BOOKING_DB, index=False)
...
# ==========================================
    ...
    if st.session_state.page == "🏠 Home":
        ...
        # लॉगिन असलेल्या युजरचा मोबाईल नंबर मिळवणे
        users_df = pd.read_csv(USER_DB)
        current_user_mob = users_df[users_df['username'] == st.session_state.user]['mobile'].values[0]

        with st.form("booking_form"):
            s = st.text_input("Pickup Point")
            d = st.text_input("Drop Point")
            v = st.selectbox("गाडी", list(RATES.keys()))
            km = st.number_input("किमी", value=50, min_value=1)
            ...
            if st.form_submit_button("Confirm Booking ✅"):
                if s and d:
                    ...
                    # डेटाबेसमध्ये बुकिंग माहिती सेव्ह करणे (मोबाईल नंबरसह)
                    pd.DataFrame([[st.session_state.user, datetime.now().strftime("%d-%m-%Y"), s, d, v, fare, pay, current_user_mob]], 
                                 columns=['username', 'date', 'from_loc', 'to_loc', 'vehicle', 'fare', 'pay_mode', 'cust_mob']).to_csv(BOOKING_DB, mode='a', header=False, index=False)
                    
                    # व्हॉट्सॲप मेसेज फॉरमॅटिंग
...
st.markdown(f"### [🚀 व्हॉट्सॲपवर पाठवा](https://wa.me/{WA_LINK_NO}?text={encoded_msg})")
        ...
        if os.path.exists(BOOKING_DB):
            ...
            st.dataframe(h[h['username'] == st.session_state.user][['date', 'from_loc', 'to_loc', 'vehicle', 'fare', 'pay_mode']], use_container_width=True)