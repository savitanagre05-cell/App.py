...
st.set_page_config(page_title="Balaji Logistics & Tours", layout="wide")
...
if not st.session_state.logged_in:
    st.title("🚩 BALAJI LOGISTICS & TOURS")
...
""", unsafe_allow_html=True)
        ...
        if st.button("Confirm Booking"):
            if not s or not d:
                ...
                new = pd.DataFrame([[
                    bid, st.session_state.user, booking_time_str,
                    s, d, v, fare, pay, mob, img_path, "Confirmed", trip_type, "Not Assigned", "Not Assigned"
        ...
        if user_df.empty:
            ...
            for idx, r in user_df.iterrows():
                ...
                <div style="background:#1e1e1e; padding:15px; border-radius:10px; margin-bottom:10px;">
                    ...
                    👨‍✈️ Driver Assigned: {r['driver_name']} ({r['driver_mobile']})<br>
                    📅 Date & Time: {r['date']}</p>
                </div>
                """, unsafe_allow_html=True)
...
"""
    ...
    # ---------------- ADMIN PAGE ----------------
        ...
        if pw == ADMIN_PASS:
            ...
            col_m2.metric("Total Revenue", f"₹{total_rev}")

            st.subheader("📋 All Bookings Database")
            st.dataframe(df)

            st.markdown("---")
            st.subheader("👨‍✈️ Assign Driver to Booking")
            if not df.empty:
                b_ids = df["booking_id"].tolist()
                selected_bid = st.selectbox("Select Booking ID", b_ids)
                d_name = st.text_input("Driver Name")
                d_mob = st.text_input("Driver Mobile Number")

                if st.button("Assign Driver"):
                    if d_name and d_mob:
                        df.loc[df["booking_id"] == selected_bid, "driver_name"] = d_name
                        df.loc[df["booking_id"] == selected_bid, "driver_mobile"] = d_mob
                        df.to_csv(BOOKING_DB, index=False)
                        st.success(f"Driver {d_name} assigned successfully to booking {selected_bid}!")
                        st.rerun()
                    else:
                        st.warning("Please enter both driver name and mobile.")