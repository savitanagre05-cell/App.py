...
MY_NO = "9767981986" 
RATE_GO, RATE_SUV = 13, 20 # ₹१३ आणि ₹२० प्रति किमी

# २. डेटा साठवणे
for k, v in {"auth":False, "pg":"Home", "hist":[], "dst":5.0, "rd":{}}.items():
    if k not in st.session_state: st.session_state[k] = v

# ३. प्रीमियम स्टाईल (बॉटम नेव्हिगेशनसाठी जागा सोडली आहे)
...
# ४. प्रोफेशनल रजिस्ट्रेशन स्क्रीन (पहिले फिचर)
if not st.session_state.auth:
    ...
    if st.button("Register & Start ➔"):
        if name and len(phone) == 10:
            ...
            st.rerun()
        else: st.error("कृपया नाव आणि १० अंकी नंबर अचूक भरा.")
else:
    # अ) होम पेज लॉजिक (नकाशा + अंतर)
    if st.session_state.pg == "Home":
        st.markdown(f"#### नमस्कार, {st.session_state.u['n']}! 👋")
        
        # नाशिकचा नकाशा (Real Map Feature)
        ...
        km = random.randint(6, 15) # रँडम किमी हिशोबासाठी
        
        st.write(f"📏 अंदाजे अंतर: **{km} किमी**")
        st.write("🚗 **गाडी निवडा (Uber प्रमाणे):**")
        c1, c2 = st.columns(2)
        
        # १० किमी असेल तर ₹१३० आणि ₹२०० आपोआप होईल
