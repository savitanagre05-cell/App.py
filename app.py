    ...
    # ४. बुकिंग Page
        ...
        if st.session_state.selected_car is None:
            car_categories = {
                ...
                "🚙 SUV": {"Ertiga": {"rate": 15, "seats": "6+1", "img": "https://imgd.aeplcdn.com/664x374/n/cw/ec/44701/ertiga-exterior-right-front-three-quarter-27.jpeg"}},
            ...
            for cat, car_list in car_categories.items():
                ...
                for c_name, c_info in car_list.items():
                    ...
                    st.subheader(f"{c_name} (₹{c_info['rate']}/km)")
            ...
            p_up, d_off = st.text_input("पिकअप ठिकाण"), st.text_input("ड्रॉप ठिकाण")
            km = st.number_input("अंदाजे KM", min_value=1, value=10)
            fare = km * st.session_state.car_rate
            st.info(f"अंदाजे भाडे: ₹{fare}")
            
            # --- पेमेंट बदल ---
            pay = st.radio("पेमेंट मोड:", ["Cash", "PhonePe"], horizontal=True)
            if pay == "PhonePe":
                st.success("📞 PhonePe Number: **9309146504**")
            ...
            if st.button("Confirm Booking ✅"):
                if p_up and d_off:
                    ...
                    st.markdown(f'<a href="https://wa.me/919767981986?text=NewBooking_{st.session_state.selected_car}_{p_up}_to_{d_off}" target="_blank" class="wa-btn">WhatsApp वर कळवा</a>', unsafe_allow_html=True)
                else: st.warning("कृपया सर्व माहिती भरा!")
            st.markdown('</div>', unsafe_allow_html=True)

    # ५. Bottom Navigation Bar
    st.markdown('<div style="height: 100px;"></div>', unsafe_allow_html=True)
    cols = st.columns(4)
    nav_items = [("🏠 Home", "Home"), ("🚕 Book", "Book"), ("📜 History", "History"), ("👤 Profile", "Profile")]
    for i, (label, pg) in enumerate(nav_items):
